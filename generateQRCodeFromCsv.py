...
def generate_qr_code_ids():
    with open('userdata.csv', newline='') as userdata:
        ...
        for row in parsed_userdata:
            ...
            BARCODE = row['BARCODE']
            NAME = row['FIRST NAME'] + " " + \
                row['MIDDLE INITIAL'] + " " + row['SURNAME']
            ROLE = row['ROLE']
            ...
            text_size = draw_underlined_text(
                draw, (0, qr_code_img.size[1]), ROLE, FONT, fill=0)

            print(text_size[1]+qr_code_img.size[1])

            draw.text((0, text_size[1]+qr_code_img.size[1]+5),
                      NAME, font=FONT2, fill=(0, 0, 0))

            padding.save(".\\generated_qr_codes\\" +
                         NAME + ' - ' + BARCODE + '.png')
...
def main():
    ...
    # company options
    parser.add_argument(
        "company", help="specify which company the qr id belongs to. ex. FCI, KSK, PROTEMPS, and TRIGOLD only.")
    # csv file
    parser.add_argument(
        "csv", help="specify the path where is the csv located")

    arguments = parser.parse_args()

    print("COMPANY " + arguments.company)
    print("CSV PATH " + arguments.csv)
